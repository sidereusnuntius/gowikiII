...
func (ma *MockActors) CacheRemoteActor(ctx context.Context, actor activitystreams.Actor) (model.Actor, error) {
...
func (ma *MockActors) Follow(ctx context.Context, follow *model.Follow) error {
	args := ma.Called(ctx, follow)
	return args.Error(0)
}

func (ma *MockActors) AcceptFollow(ctx context.Context, actorIRI, followIRI string) error {
	args := ma.Called(ctx, actorIRI, followIRI)
	return args.Error(0)
}
